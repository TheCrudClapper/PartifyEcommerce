...
public interface IOfferDeliveryTypeRepository
{
    ...
    Task AddAsync(OfferDeliveryType entity, CancellationToken cancellationToken = default);
    ...
    /// <returns>A task that represents the asynchronous operation.</returns>
    Task AddRangeAsync(IEnumerable<OfferDeliveryType> entities, CancellationToken cancellationToken = default);
}
