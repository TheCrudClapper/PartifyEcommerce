...
public class CachingHelper : ICachingHelper
{
    ...
    public async Task<(bool Found, T? Value)> GetCachedObject<T>(string cacheKey, CancellationToken cancellationToken)
    {
    ...
    public async Task CacheObject<T>(T obj, string cacheKey, DistributedCacheEntryOptions options, CancellationToken cancellationToken)
    {
